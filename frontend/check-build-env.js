...
function checkBuildEnvironment() {
  console.log(chalk.blue('Checking build environment for Windows installer...'));
  
  const platform = os.platform();
  
  if (platform === 'win32') {
    console.log(chalk.green('✅ Running on Windows - native build is supported'));
    return true;
  } else {
    console.log(chalk.yellow('⚠️ Not running on Windows. You can still build using:'));
    ...
    if (process.env.FORCE_BUILD) {
      console.log(chalk.yellow('FORCE_BUILD is set, continuing with build...'));
      return true;
    }
